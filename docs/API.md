...
| POST | `/journals/{id}/reverse` | `journals:post` | Create one posted opposite entry for a manual journal |

Reversal journals, already reversed journals, and journals generated by an
invoice or payment return 409. Source-document correction requires a workflow
that updates the source document and ledger together; direct generic reversal
