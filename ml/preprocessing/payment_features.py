...
PAYMENT_FEATURES = [
    "amount", "prior_invoice_count", "prior_average_amount", "prior_average_delay",
    "prior_late_rate", "prior_paid_amount", "prior_invoice_frequency",
    "prior_payment_frequency", "outstanding_at_prediction", "customer_tenure_days",
]


def build_payment_features(events: pd.DataFrame, delay_days: int = 7) -> pd.DataFrame:
    required = {"customer_id", "invoice_id", "invoice_date", "due_date", "payment_date", "amount"}
    ...
    for customer_id, group in ordered.groupby("customer_id", sort=True):
        ...
        for _, current in group.iterrows():
            ...
            completed = [item for item in history if pd.Timestamp(item["payment_date"]) < prediction_time]
            delays = [
                (pd.Timestamp(item["payment_date"]) - pd.Timestamp(item["due_date"])).days
            ...
            target_delay = (
                pd.Timestamp(current["payment_date"]) - pd.Timestamp(current["due_date"])
            ).days
            ...
            rows.append({
                ...
                "prior_average_amount": (
                    sum(float(item["amount"]) for item in history) / len(history) if history else 0.0
                ),
                "prior_average_delay": sum(delays) / len(delays) if delays else 0.0,
                "prior_late_rate": (
                    sum(value > delay_days for value in delays) / len(delays) if delays else 0.0
                ),
                "prior_paid_amount": sum(float(item["amount"]) for item in completed),
