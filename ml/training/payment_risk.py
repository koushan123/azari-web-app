...
class PaymentRiskModel:
    ...
    def predict(self, features: dict[str, float]) -> RiskPrediction:
        ...
        contributions = {
            ...
            for name, importance in zip(PAYMENT_FEATURES, self.estimator.feature_importances_, strict=True)
        }
        ordered = dict(sorted(contributions.items(), key=lambda item: abs(item[1]), reverse=True))
        classification = "high" if probability >= self.risk_threshold else "low"
        return RiskPrediction(classification, probability, self.metadata.model_version, ordered)
...
def train_payment_risk_model(
    events: pd.DataFrame, *, seed: int, delay_days: int = 7,
    risk_threshold: float = 0.5,
    model_version: str = "payment-risk-v1",
) -> tuple[PaymentRiskModel, dict[str, float], tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]]:
    features = build_payment_features(events, delay_days)
    train, validation, test = temporal_split(features)
    estimator = RandomForestClassifier(
        n_estimators=180, max_depth=8, min_samples_leaf=3, class_weight="balanced",
    ...
    metrics = {
        ...
        "validation_roc_auc": float(roc_auc_score(
            validation["delayed"], estimator.predict_proba(validation[PAYMENT_FEATURES])[:, 1]
        )),
    }
    metadata = make_metadata(
        pipeline="payment_delay_risk", model_version=model_version, data=events,
        features=PAYMENT_FEATURES, seed=seed,
        config={"delay_days": delay_days, "risk_threshold": risk_threshold}, metrics=metrics,
    )
    baseline = {name: float(train[name].mean()) for name in PAYMENT_FEATURES}
    return PaymentRiskModel(estimator, metadata, baseline, risk_threshold), metrics, (
        train, validation, test
    )


def save_payment_risk_model(model: PaymentRiskModel, path: Path) -> None:
    save_artifact(path, {"estimator": model.estimator, "baseline": model.baseline}, model.metadata)
...
def load_payment_risk_model(path: Path) -> PaymentRiskModel:
    ...
    return PaymentRiskModel(
        state["estimator"], metadata, state["baseline"],
